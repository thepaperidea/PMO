...
class Process {
  ...
  public function columnexplodes($data,$seperator = "_"){
  ...
  public function getSingle($table,$i,$column = 'id',$orderBy,$order){
    global $dbh;
    ...
    if($row){
      ...
      foreach ($row as $key => $value) {
        if(!is_numeric($key)){
          $explode = Process::columnexplodes($key);
          if($explode[1]=='id')
          $value = Process::getSingle($explode[0],$value);
          elseif($explode[1]=='ids')
          $value = Process::getMultiple($explode[0],$value);
